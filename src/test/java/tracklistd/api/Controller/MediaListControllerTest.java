...
public class MediaListControllerTest {
        ...
        void setUp() {
                ...
                testMediaList = Mockito.spy(new MediaList(testUser, ListType.ALBUM, "My Album List", Privacy.PUBLIC,
                ...
                testResponseDto = new MediaListResponseDto(
                                ...
                                Set.of(new MediaMinDTO("media-1", "Media Title", "Artist Name", "album", "cover-url",
                                                180000, "3m")),
                ...
                // Mock padrão para o LikeRepository evitar NullPointerException nas
                // verificações internas do Controller
                when(likeRepository.existsByUserIdAndPublicationId(anyLong(), anyLong())).thenReturn(false);
        }
        ...
        void createMediaList_deveRetornar201_quandoDadosValidos() throws Exception {
                MediaListRequestDto request = new MediaListRequestDto(ListType.ALBUM, "My Album List", false,
                                Privacy.PUBLIC, null, null, null, null);

                when(mediaListService.createMediaList(any(), any(), any(), any(), any(), any(), any(), any()))
                                .thenReturn(testMediaList);
                // Ajustado para capturar o 4º parâmetro boolean (likedByMe)
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
                when(mediaListMapper.toOwnerResponseDTO(eq(testMediaList), eq(testResponseDto)))
                                .thenReturn(testOwnerResponseDto);
        ...
        void getOneListByUser_deveRetornar200Owner_quandoUsuarioForDono() throws Exception {
                ...
                // Corrigido para corresponder a assinatura correta com anyBoolean()
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
                when(mediaListMapper.toOwnerResponseDTO(testMediaList, testResponseDto))
                                .thenReturn(testOwnerResponseDto);
                ...
                mockMvc.perform(get("/api/mediaList/10")
                                ...
                                // Corrigido a validação que falhava por causa da estrutura do DTO de resposta
                                // do Dono
        ...
        void getOneListByUser_deveRetornar200Public_quandoUsuarioNaoForDono() throws Exception {
                when(mediaListService.getMediaListById(10L)).thenReturn(testMediaList);
                // Corrigido adicionando anyBoolean()
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
        ...
        void getOneListByUser_deveRetornar200Public_quandoNaoAutenticado() throws Exception {
                when(mediaListService.getMediaListById(10L)).thenReturn(testMediaList);
                // Ajustada assinatura e removido boolean "false" perdido no retorno antigo do
                // mock
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
        ...
        void getAllListByUser_deveRetornarTodasComoOwner_quandoUsuarioForDono() throws Exception {
                ...
                // Ajustada assinatura para o método que monta a lista do DTO pública
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
                when(mediaListMapper.toOwnerResponseDTO(testMediaList, testResponseDto))
                                .thenReturn(testOwnerResponseDto);
        ...
        void getAllListByUser_deveRetornarTodasComoPublic_quandoUsuarioNaoForDono() throws Exception {
                when(userService.findUserById(1L)).thenReturn(testUser);
                when(mediaListService.getAllByUser(testUser)).thenReturn(List.of(testMediaList));
                // Ajustado adicionando anyBoolean()
                when(mediaListMapper.toResponseDto(eq(testMediaList), anyLong(), anyInt(), anyBoolean()))
                                .thenReturn(testResponseDto);
        ...
        void editMediaListName_deveRetornar202_quandoDonoEdita() throws Exception {
                MediaListEditRequestDto.EditNameRequestDto request = new MediaListEditRequestDto.EditNameRequestDto(
                                "New List Name");
        ...
        void editMediaListPrivacy_deveRetornar202_quandoDonoEdita() throws Exception {
                MediaListEditRequestDto.EditPrivacyRequestDto request = new MediaListEditRequestDto.EditPrivacyRequestDto(
                                Privacy.PRIVATE);
